...
#args로 받는 것들에 True, False를 받는 것에 대한 보조 함수로 추정
def str2bool(v):
...
#실행 파일에서 인자로 받는 것들 args.~의 변수로 밑에서 활용 됨 
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Postprocess'):
        ...
        detected_area = np.zeros((ay,ax))
        for i in range(len(t[0])):
            if int(t[0][i]) == 59: 
                detected_area += t[3][i].cpu().detach().numpy()
        if np.sum(detected_area) == 0:
            print('카메라에 원하는 대상이 없습니다. 다시 시도해주세요.')
        else:
            np.savetxt('area.csv', detected_area, delimiter = ',')
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            # if cfg.dataset.class_names[classes[j]] == 'car': # 바인딩 박스 조건  우리는 바인딩 박스를 사용하지 않으므로 주석처리
            ...
            if cfg.dataset.class_names[classes[j]] == 'cup': # txt표시 조건
                if args.display_text:
                    num += 1
                    _class = cfg.dataset.class_names[classes[j]]
                    text_str = '%s_%s: %.2f' % (_class, str(num),score) if args.display_scores else _class
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    cv2.imwrite("current_area.jpg", img_numpy)
...
def area_setting():
    setting_way = input('영역설정 방법을 입력 후 엔터키를 눌러주세요\n1.웹캠으로 캡처 후 영역 설정\n2.이미지 파일을 통해 영역 설정\n')
    if setting_way == '1':
        camera = cv2.VideoCapture(0)
        ret, image = camera.read()
        save_img_path = input('실행 파일과 같은 폴더에 저장될 이미지의 이름을 확장자까지 포함하여 입력해주세요.\n0을 입력시 current_shot.jpg로 저장됩니다.\n')
        if save_img_path == '0':
            save_img_path = 'current_shot.jpg'
        cv2.imwrite(save_img_path, image)
        camera.release()
        cv2.destroyAllWindows()
        image_path = save_img_path
    elif setting_way == '2':
        input_image_path = input('실행 파일과 같은 폴더에 있는 사용할 이미지 파일의 이름을 확장자까지 포함하여 입력해주세요.\n')
...
def realtime_detecting():
    body_estimation = Body('./model/body_pose_model.pth')
    target_area = np.loadtxt('area.csv', delimiter = ',')
    ...
    while True:
        ...
        RHip_point = util.get_RHip_point()
        LHip_point = util.get_LHip_point()
        RKnee_point = util.get_RKnee_point()
        LKnee_point = util.get_LKnee_point()

        print('RHip point: ',RHip_point)
        print('LHip point: ',LHip_point)
        print('RKnee point: ',RKnee_point)
        print('LKnee point: ',LKnee_point)

        if len(RHip_point) == 0 or len(LHip_point) == 0 or len(RKnee_point) == 0 or len(LKnee_point) == 0:
            print('해당 포인트가 화면에 잡히지 않았습니다.')

        else:
            state = []

            RHip_x, RHip_y = int(RHip_point[0][0]),int(RHip_point[0][1])
            LHip_x, LHip_y = int(LHip_point[0][0]),int(LHip_point[0][1])
            RKnee_x, RKnee_y = int(RKnee_point[0][0]),int(RKnee_point[0][1])
            LKnee_x, LKnee_y = int(LKnee_point[0][0]),int(LKnee_point[0][1])
            
            if target_area[RHip_y][RHip_x] and target_area[LHip_y][LHip_x] and target_area[RKnee_y][RKnee_x] and target_area[LKnee_y][LKnee_x] == 1:
                object_state = 0
            ...
            if sum(state) > 0:
                ...
                state_time += 1
                if state_time >= 3:
                    playsound("./warning_bed.mp3")

                    # push_service = FCMNotification(api_key="") # 파이어베이스 서버 api값 입력
                    # registration_id = "" # 기기 토큰값 입력 
                    # message_title = "낙상 위험 경고"
                    # message_body = "낙상 방지를 위해 내용을 확인해주세요."
        ...
        RShoulder_point = util.get_RShoulder_point()
        LShoulder_point = util.get_LShoulder_point()
        Nose_point = util.get_Nose_point()
        Neck_point = util.get_Neck_point()
        
        if len(RShoulder_point) == 1 and len(LShoulder_point) == 1 and len(Nose_point) == 1 and len(Neck_point) == 1:
            RShoulder_x, RShoulder_y = RShoulder_point[0]
            LShoulder_x, LShoulder_y = LShoulder_point[0]
            Nose_x, Nose_y = Nose_point[0]
            Neck_x, Neck_y = Neck_point[0]

            if (LShoulder_x-RShoulder_x)**2 + (LShoulder_y-RShoulder_y)**2 >= ((Nose_x-Neck_x)**2 + (Nose_y-Neck_y)**2)*1.3:
            ...
            elif (LShoulder_x-RShoulder_x)**2 + (LShoulder_y-RShoulder_y)**2 < ((Nose_x-Neck_x)**2 + (Nose_y-Neck_y)**2)*1.3:
                if Nose_x > Neck_x and Nose_y > Neck_y:
                    ...
                    current_pose = 'R'
                    print('오른쪽')
            if current_pose == post_pose:
                pose_score += 1
                if pose_score > 10:
                    print("욕창 방지를 위해 자세를 바꿔주세요")
                    playsound("./warning_bedsore.mp3")

                    # push_service = FCMNotification(api_key="") #파이어베이스 서버 api키 입력
                    # registration_id = "" #기기 토큰 값 입력 
                    # message_title = "욕창 위험 경고"
                    # message_body = "욕창 방지를 위해 내용을 확인해주세요."
                    # result = push_service.notify_single_device(registration_id=registration_id, message_title=message_title, message_body=message_body)
                    # print(result)
        ...
        cv2.imshow('Realtime Detecting', canvas) # 작업한 영상을 출력
        if cv2.waitKey(1) & 0xFF == ord('q'): # q를 입력시 종료
            break
...
def video_detecting():
    target_area = np.loadtxt('area.csv', delimiter = ',')
    input_video_path = input('사용할 동영상 파일의 이름을 확장자까지 포함하여 입력해주세요. 동영상 파일은 실행파일과 같은 폴더에 있어야 합니다.\n')
    ...
    # 비디오의 속성 값을 받아오기 정수 형태로 변환하기 위해 round
    ...
    while True:                 
        ret, frame = cap.read()
        if not ret:             #frame당 작업을 하고 작업할 프레임이 존재하지 않을 때 종료
            break
        ...
        if len(RHip_point) == 0 or len(LHip_point) == 0 or len(RKnee_point) == 0 or len(LKnee_point) == 0:
            cv2.rectangle(canvas, (15,200), (280,140), (255,255,255), -1)
            cv2.putText(canvas, "NOT Detected", (25, 183), cv2.FONT_HERSHEY_DUPLEX, 1.15,(0,0,0), 2, cv2.LINE_AA)
            ...
            if target_area[RHip_y][RHip_x] and target_area[LHip_y][LHip_x] and target_area[RKnee_y][RKnee_x] and target_area[LKnee_y][LKnee_x] == 1:
                cv2.rectangle(canvas, (15,200), (280,140), (255,255,255), -1)
                cv2.putText(canvas, "Safe", (100, 183), cv2.FONT_HERSHEY_DUPLEX, 1.15,(255,0,0), 2, cv2.LINE_AA)
            else:
                cv2.rectangle(canvas, (15,200), (280,140), (255,255,255), -1)
                cv2.putText(canvas, "Waring!!", (75, 183), cv2.FONT_HERSHEY_DUPLEX, 1.15,(0,0,255), 2, cv2.LINE_AA)
        ...
        front_pose_sentence = "Front"
        back_pose_sentence = "Back"
        right_pose_sentence = "Right"
        left_pose_sentence = "Left"
        ...
        if len(RShoulder_point) == 1 and len(LShoulder_point) == 1 and len(Nose_point) == 1 and len(Neck_point) == 1:
            ...
            if (LShoulder_x-RShoulder_x)**2 + (LShoulder_y-RShoulder_y)**2 >= ((Nose_x-Neck_x)**2 + (Nose_y-Neck_y)**2)*1.3:
                if Nose_y > Neck_y and LShoulder_x > RShoulder_x :
                    current_pose = 'B'
                    cv2.rectangle(canvas, (15,280), (280,220), (255,255,255), -1)
                    cv2.putText(canvas, back_pose_sentence, (105, 263), cv2.FONT_HERSHEY_DUPLEX, 1.15,(0,0,0), 2, cv2.LINE_AA)
                    print('엎드려 누워있습니다.')

                elif Nose_y > Neck_y and LShoulder_x <= RShoulder_x :
                    current_pose = 'F'
                    cv2.rectangle(canvas, (15,280), (280,220), (255,255,255), -1)
                    cv2.putText(canvas, front_pose_sentence, (100, 263), cv2.FONT_HERSHEY_DUPLEX, 1.15,(0,0,0), 2, cv2.LINE_AA)
                    print('똑바로 누워있습니다.')
                ...
                if Nose_x > Neck_x and Nose_y > Neck_y:
                    current_pose = 'R'
                    cv2.rectangle(canvas, (15,280), (280,220), (255,255,255), -1)
                    cv2.putText(canvas, right_pose_sentence, (100, 263), cv2.FONT_HERSHEY_DUPLEX, 1.15,(0,0,0), 2, cv2.LINE_AA)
                    print('오른쪽으로 누워있습니다.')

                elif Nose_x > Neck_x and Nose_y <= Neck_y:
                    current_pose = 'L'
                    cv2.rectangle(canvas, (15,280), (280,220), (255,255,255), -1)
                    cv2.putText(canvas, left_pose_sentence, (110, 263), cv2.FONT_HERSHEY_DUPLEX, 1.15,(0,0,0), 2, cv2.LINE_AA)
                    print('왼쪽으로 누워있습니다.')
            ...
            if current_pose == post_pose:
                pose_score += 1
                if pose_score > 10:
                    cv2.rectangle(canvas, (15,360), (280,300), (255,255,255), -1)
                    cv2.putText(canvas, "Change Position", (18, 343), cv2.FONT_HERSHEY_DUPLEX, 1,(0,0,0), 2, cv2.LINE_AA)
                    print("욕창 방지를 위해 자세를 바꿔주세요")
        ...
        cv2.imshow('Video Detecting per frame', canvas)

        out.write(canvas) # 영상 데이터만 저장

        if cv2.waitKey(delay) == 'q': # q를 누르면 강제 종료
            break
...
def main():
    while True:
        print('메뉴를 선택해주세요.\n 1. 영역 설정  2. 실시간 탐지  3. 동영상 탐지  4. 종료')
        sel = input('번호 입력 후 엔터 : ')
        ...
        if sel == '1':
            ...
            print('실시간 탐지를 실행합니다.\n실행까지 5~20초 정도의 시간이 걸립니다.\n탐지 종료를 원하실 경우 q키를 눌러주세요.')
            realtime_detecting()
